...
class Predict:
    ...
    def is_valid_match(self, filtered_match):
        MIN_ODD, MID_ODD, MAX_ODD, MIN_PROB = 1.12, 1.4, 1.9, 85
    ...
    def __call__(self):
        ...
        if new_matches > 0:
            OneSignal().send_push_notification(f"{len(new_matches)} New Predictions have Just been Posted! Refresh to see them (Pull to Refresh)!!!")
